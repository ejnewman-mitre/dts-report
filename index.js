...
function glprocess(records) {
  let data = {
    author:
      '<a href="mailto:ejnewman@mitre.org?subject=DTS Report">Eric Newman</a>',
    recipient: "TBD",
    bads: glbads,
  ...
  Object.keys(glbads).forEach(function (key) {
    ...
    if (p) {
      ...
      sendEmail(
        ...
        data.recipient,
        key,
        "Group Lead DTS Updates Report Summary"
      );
    }
  });
}
...
function eprocess(records) {
  ...
  $("#ContentPlaceHolder1_ctl09_tblDeptDetail tbody > tr").each(
    (index, element) => {
      user =
        $(
          "#ContentPlaceHolder1_ctl09_tblDeptDetail > tbody > tr:nth-child(" +
            index +
            ") > th:nth-child(1)"
        )
          .find("a")
      ...
      if (user) {
        ...
        switch (problem) {
          case "images/redx.png":
            ...
            let baduser =
              user + "</td><td>" + machname.trim() + "</td><td>" + mach.trim();
            if (!badz[email]) {
              badz[email] = [];
            }
            ...
            if (!glbads[found.manager]) {
              glbads[found.manager] = [];
            }
